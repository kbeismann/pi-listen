...
| `/voice` | Toggle on/off |
| `/talk on` / `off` | Start or stop hands-free conversation |
...
- Its conversational system prompt optimizes answers for auditory understanding:
  responses should be easy to follow on one hearing, lead with conclusions, keep
  one main idea per sentence, and use explicit transitions and referents. It
  defaults to roughly three or four spoken sentences without headings, lists,
  tables, or code-heavy text. The default is soft: explicit requests for detail
  and answers that genuinely require more context can run longer.
...
`generativelanguage.googleapis.com` machine entry to `~/.authinfo`, then set:
