...
// Keep spoken answers understandable on one hearing and longer tasks
// conversational through prompt guidance alone. The acknowledgement remains
// ordinary assistant text; first-text latency still depends on the active model
// rather than a timer or a separate model call.
...
export function createTalkMode(pi: ExtensionAPI, dependencies: TalkModeDependencies) {
	...
	function maybeSpeakMessage(message: any, isFinal: boolean): void {
		...
		if (!newText) return;
		const useGemini = state.config?.talk.ttsBackend === "gemini";

		let speakLength = 0;
		if (isFinal) speakLength = newText.length;
		else if (useGemini) {
			// A streamed assistant message can otherwise consume several remote
			// requests in one spoken turn. Wait for message_end so Gemini receives
			// exactly one complete request; local synthesis keeps its lower-latency
			// sentence streaming below.
			return;
		} else {
		...
		const prepared = prepareForSpeech(chunk, {
			// The complete Gemini message replaces several formerly bounded
			// fragments, so preserve the same total spoken content in its one call.
			maxChars: useGemini ? Number.POSITIVE_INFINITY : 2_000,
